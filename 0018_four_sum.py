...
class Solution:
    ...
    def kSum(self, nums: List[int], k: int, target: int, b: int = 0,
             t: int = None, sorted: bool = False) -> List[List[int]]:
        ...
        if k == 2:
            self.twopointer(nums, target, res, b, t, ())
        else:
            self.outerloop(nums, k, target, res, b, t, (), k % 2 == 0)

        return res

    def outerloop(self, nums: List[int], k: int, target: int,
                  res: List[tuple[int, ...]], b: int, t: int,
                  vs: tuple[int, ...], right: bool) -> None:
        ...
        if right:
            ...
            for i, v in enumerate(nums[s: e], s + 1):
                if p == v: continue
                p = v
                self.outerloop(nums, k - 1, target - v, res, i, t, vs + (v,),
                               False)
        else:
            ...
            if k > 3:
                for i, v in enumerate(reversed(nums[s: e]), 1 - e):
                    if p == v: continue
                    p = v
                    self.outerloop(nums, k - 1, target - v, res, b, - i,
                                   vs + (v,), True)
            else:
                for i, v in enumerate(reversed(nums[s: e]), 1 - e):
                    if p == v: continue
    ...
    def twopointer(self, nums: List[int], target: int, res: List[List[int]],
                   b: int, t: int, vs: tuple[int, ...]) -> None:
...
if __name__ == '__main__':
    test = Solution()
    assert test.fourSum([1, 0, -1, 0, -2, 2], target=0) == [[2, 1, -1, -2],
                                                            [2, 0, 0, -2],
                                                            [1, 0, 0, -1]]
